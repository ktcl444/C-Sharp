	...
	public class DefaultTranscoder : ITranscoder
	{
		...
		protected virtual object Deserialize(CacheItem item)
		{
			...
			switch (code)
			{
                ...
                case TypeCode.Object:
                    {
                        byte[] tmp = new byte[data.Array.Length - data.Offset];
                        Array.Copy(data.Array, data.Offset, tmp, 0, tmp.Length);
                        Byte[] b = decompress(tmp);
                        return this.DeserializeObject(new ArraySegment<byte>(b, 0, (int)b.Length));
                    }
		...
		protected virtual ArraySegment<byte> SerializeObject(object value)
		{
			using (var ms = new MemoryStream())
			{
                ...
                uint compressionThreshold = 1024 * 128; //128kb
                if (bytes.Length > compressionThreshold)
                {
                    bytes = compress(bytes);
                }
                return new ArraySegment<byte>(bytes, 0, (int)bytes.Length);
        ...
        protected virtual byte[] compress(byte[] bytes)
        {
            ...
            {
                using (System.IO.Compression.DeflateStream gzs = new System.IO.Compression.DeflateStream(ms, System.IO.Compression.CompressionMode.Compress, false))
                {
