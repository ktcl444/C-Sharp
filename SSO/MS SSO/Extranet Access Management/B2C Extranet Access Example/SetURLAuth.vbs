...
'
' Confirmation (Get and display URLAuth settings)
'
Dim MB_Obj

Set MB_Obj= GetObject("IIS://localhost/" & objArgs(0))

wscript.echo vbnewline & "URLAuth MetaBase values for " & objargs(0)

Wscript.echo "Authorization Manager store URL: " & MB_Obj.AzStoreName

wscript.echo "Scope Name assigned: " & MB_Obj.AzScopeName

wscript.echo "URLAuth enabled: " & MB_Obj.AzEnable

wscript.echo "Impersonate setting: " & MB_Obj.AzImpersonationLevel
