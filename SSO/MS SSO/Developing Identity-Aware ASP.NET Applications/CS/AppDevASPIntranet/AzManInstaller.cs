...
namespace Contoso.AppDevASPSample
{
	...
	{
		private const String AZ_POLICY_URL = @"msxml://C:\AppDevASPIntranet.xml";
		private const String APPLICATION_NAME = "AppDevASPIntranet";
		...
		private void CreateRoles(IAzApplication app)
		{
			...
			try
			{
				scope = app.OpenScope("All", null);
			}
			catch
			{
				scope = app.CreateScope("All", null);
