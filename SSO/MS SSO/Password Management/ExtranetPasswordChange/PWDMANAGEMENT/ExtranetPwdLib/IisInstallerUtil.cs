...
{
	/// <summary>
	/// Represents tools that wrap common IIS configuration code.
	/// Used by KerberosAuthInstaller and FormsAuthInstaller.
	/// </summary>
	public class IisInstallerUtil
	{
		...
		public static Int32 FindServerNum(String port)
		{
			...
			{
				IISObjectPath = 
					"IIS://" + machineName + "/W3SVC/" + servers[i].ToString();
				IISObject = GetIisObject(IISObjectPath);
			
				//Gets the Port Number of the current IISObject.
				serverBindings = 
					(String)IISObject.Properties["ServerBindings"].Value;
				serverBindingParts = 
					serverBindings.Split(new char[] {':'});
				...
				if (port == serverPort)
					if(Convert.ToInt32(IISObject.Properties["ServerState"].Value)
			...
			{
				case 0:
					throw new Exception("No Active Servers with the requested port"
						+ "were found. Port=" + port + ". ");
				case 1:
					return Convert.ToInt32(servers[0]);
				default:
					throw new Exception("More than one Active servers with " +
						"the requested port were found. Port=" + port + ". ");
			}
		}

		/// <summary>
		/// Verify the existence of a certain object in the Directory entry.
		/// </summary>
		/// <param name="strFullObjectPath">Full path of the Object
		/// </param>
		/// <returns>Returns the directory object for the given path</returns>
