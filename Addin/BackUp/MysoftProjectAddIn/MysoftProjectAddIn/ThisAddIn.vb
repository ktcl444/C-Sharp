...
#End Region
        ...
        Try
            ...
            For i = 0 To fieldName.Length - 1
                ...
                If Not isColumnExist Then
                    ...
                    'Application.TableEdit(Name:="资源名称", TaskTable:=True, NewName:="", FieldName:="", NewFieldName:=fieldName(i), Title:="", Width:=10, Align:=2, ShowInMenu:=True, LockFirstColumn:=True, DateFormat:=255, RowHeight:=1, ColumnPosition:=-1, AlignTitle:=1)
                    'Application.TableApply(Name:="资源名称")

                    'Application.TableEdit(Name:="资料编辑", TaskTable:=True, NewName:="", FieldName:="", NewFieldName:=fieldName(i), Title:="", Width:=10, Align:=2, ShowInMenu:=True, LockFirstColumn:=True, DateFormat:=255, RowHeight:=1, ColumnPosition:=-1, AlignTitle:=1)
                    'Application.TableApply(Name:="资料编辑")
            ...
            Next
        Catch ex As Exception
            MessageBox.Show(ex.Message & Environment.NewLine & ex.StackTrace, "编制工作项错误", _
                         MessageBoxButtons.OK, MessageBoxIcon.Error)
        End Try
    End Sub

    Private Sub Test(ByVal tableName As String)
        Try
            Application.TableEdit(Name:=tableName, TaskTable:=True, NewName:="", FieldName:="", NewFieldName:="007", Title:="", Width:=10, Align:=2, ShowInMenu:=True, LockFirstColumn:=True, DateFormat:=255, RowHeight:=1, ColumnPosition:=-1, AlignTitle:=1)
            Application.TableApply(Name:=tableName)
        Catch ex As Exception
            MessageBox.Show(ex.Message & Environment.NewLine & ex.StackTrace, "编制工作项错误", _
                         MessageBoxButtons.OK, MessageBoxIcon.Error)
        End Try
    ...
    Private Sub UploadProject(ByVal datafile As String, ByVal infofile As String)
        ...
        Dim s As String
        Dim bytes() As Byte
        bytes = System.Text.Encoding.UTF8.GetBytes(xmlInfo.OuterXml)
